...
//                         }
//                     } ></div>
//                     <Tooltip className='myClass' id={"tootip-" + idx} effect="float" border="true" delayShow={100}>{this.state.isBusy ? "wait" : value} </Tooltip>
//                 </div>
//             );
//         });
...
class SortInterface extends Component { 
    ...
    resetArray() {
        const new_arr = [];
        const { min_val, max_val, len } = this.state;
    ...
    handleMergeSort() {
        ...
        const result = mergeSort(this.state.arr);
        const { indexValuePairs, arr: tmp_arr } = result;
        const bars = document.getElementsByClassName("bar");
        ...
        indexValuePairs.map((pair, index) => {
            setTimeout(() => { 
                ...
                if (index === indexValuePairs.length - 1) {
                    this.setState({ arr: tmp_arr, isBusy: false });
                    Array.from(bars).forEach(bar => bar.style.backgroundColor = this.state.barColor);
                }
            }, index * this.state.timePerBar);
        });
    }
    ...
    handleQuickSort(isRandomized) {
        ...
        const result = isRandomized ? randomizedQuickSort(this.state.arr) : quickSort(this.state.arr);
        const { swap_triplets, arr: tmp_arr } = result;
        const bars = document.getElementsByClassName("bar");
    ...
    handleInsertionSort() {
        ...
        const result = insertionSort(this.state.arr);
        const { triplets, arr: tmp_arr } = result;
        const bars = document.getElementsByClassName("bar");
    ...
    renderBars() {
        const bar_seq = this.state.arr.map((value, idx) => (
            <div key={idx}>
                <div data-tip="" data-for={"tooltip-" + idx} className="bar" style={{
                    width: `${89 * 0.7 / this.state.arr.length}vw`,
                    backgroundColor: this.state.barColor,
                    marginLeft: `${89 * 0.3 / this.state.arr.length}vw`,
                    height: `${value}px`,
                ...
                <Tooltip className='myClass' id={"tooltip-" + idx} effect="float" border={true} delayShow={100}>{this.state.isBusy ? "wait" : value} </Tooltip>
            </div>
        ));
        ...
        return (
            <div className="col-12 col-md-11 border d-flex flex-wrap align-items-end justify-content-center"
                style={{
                    "backgroundImage": `linear-gradient(to bottom right, ${this.state.bgColor0}, ${this.state.bgColor1})`,
                    height: `${this.state.max_val + 50}px`
                }}>
                {bar_seq}
            </div>
    ...
    onSpeedChange(value) {
        ...
        this.setState({ timePerBar: time });
    }

    onBarCountChange(value) {
        this.setState({ len: value }, () => this.resetArray());
    }

    render() {
        return (
            <div>
                <div className="row bg-near-black justify-content-center">
                    <hr />
                    {this.renderBars()}
                </div>

                <div className="row d-flex justify-content-center align-items-center bg-navy">
                    <div className="col-8 col-md-6" style={{ fontFamily: "jasmine", fontWeight: "bold" }}>
                        <Button data-tip={desc_reset} data-for="tip-reset" id="btn-reset" className="ma1 grow dib bw2 shadow-5" color="primary" disabled={this.state.isBusy} onClick={() => this.resetArray()}>Reset Array</Button>{' '}
                        <Button data-tip={desc_mergeSort} data-for="tip-merge" id="btn-mergeSort" className="ma1 grow dib bw2 shadow-5" color="success" disabled={this.state.isBusy} onClick={() => this.handleMergeSort()}>Merge Sort</Button>{' '}
                        <Button data-tip={desc_quickSort} data-for="tip-quick" id="btn-quickSort" className="ma1 grow dib bw2 shadow-5" color="success" disabled={this.state.isBusy} onClick={() => this.handleQuickSort(false)}>Quick Sort</Button>{' '}
                        <Button data-tip={desc_randomQuickSort} data-for="tip-random-merge" id="btn-randQuickSort" className="ma1 grow dib bw2 shadow-5" color="success" disabled={this.state.isBusy} onClick={() => this.handleQuickSort(true)}>Randomized Quick Sort</Button>{' '}
                        ...
                        <Tooltip className='myClass' id="tip-reset" effect="float" border={true} delayShow={200}>{desc_reset}</Tooltip>
                        <Tooltip className='myClass' id="tip-merge" effect="float" border={true} delayShow={200}>{desc_mergeSort}</Tooltip>
                        <Tooltip className='myClass' id="tip-quick" effect="float" border={true} delayShow={200}>{desc_quickSort}</Tooltip>
                        <Tooltip className='myClass' id="tip-random-merge" effect="float" border={true} delayShow={200}>{desc_randomQuickSort}</Tooltip>
                        <Tooltip className='myClass' id="tip-insertion" effect="float" border={true} delayShow={200}>{desc_insertSort}</Tooltip>
                    </div>

                    <div className="grow bg-aqua shadow b ba ma3 col-6 col-md-4 justify-content-center mt-3">
                        <p style={{ fontFamily: "jasmine", fontWeight: "bold" }}>Speed </p>
                        <Slider
                            defaultValue={50}
                            aria-labelledby="speed"
                            min={5}
                            max={100}
                            valueLabelDisplay="auto"
                            onChange={(event, value) => this.onSpeedChange(value)}
                            disabled={this.state.isBusy}
                        />
                        
                        <p style={{ fontFamily: "jasmine", fontWeight: "bold" }}>Number of bars </p>
                        <Slider
                            defaultValue={100}
                            aria-labelledby="Bars"
                            min={20}
                            max={200}
                            valueLabelDisplay="auto"
                            onChange={(event, value) => this.onBarCountChange(value)}
                            disabled={this.state.isBusy}
                        />
                        <br /><br />
                    </div>
